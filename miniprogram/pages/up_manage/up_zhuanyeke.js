Page({
  ...
  data: {
    ...
    //下拉列表的数据
    selectData: ["计算机","教育学","心理学","历史学","农学","农学","西医综合","中医综合","法律硕士"],
    index: 0,//选择的下拉列表下标
    paperurl:"",
    tempfile_url0:"",//临时
    tempfile_url:"",//存到数据库
...
upload_paper_yun(){
   wx.cloud.uploadFile({  
    //存储的路径 科目分类/时间+文件名
    cloudPath:"ZhuanYeKe"+"/"+this.data.selectData[this.data.index]+'/'+new Date().getTime()+this.data.tempfile_name,
...
// 把文件信息存储到云数据库
  add_paperList(){
    ...
    wx.cloud.database().collection("ZhuanYeKe").add({
      data:{
